...
ActivityMonitor.prototype =
{
  ...
  getIdleTime: function getIdleTime()
  {
    var that = this;
    var X = null;
    return that.initialised.then(function()
    {
      X = that.display.client;
      var Xrequire = Promise.denodeify(X.require);
      return Xrequire.call(X, 'screen-saver');
    }).then(function(SS)
    {
      var SSQueryInfo = Promise.denodeify(SS.QueryInfo);
      return SSQueryInfo.call(SS, that.display.screen[0].root);
    }).then(function(info)
    {
      return info.idle;
    });
